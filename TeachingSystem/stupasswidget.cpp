...
StuPassWidget::StuPassWidget(QString sid,QWidget *parent) : QWidget(parent)
{
    stuID=sid;
    init_layout();
    init_connect();
...
void StuPassWidget::init_layout()
{
...
void StuPassWidget::init_connect()
{
    connect(button_confirm,&QPushButton::clicked,this,&StuPassWidget::click_confirm);
    connect(button_reset,&QPushButton::clicked,this,&StuPassWidget::click_reset);
}
void StuPassWidget::click_confirm()
{
    ...
    else
    {
        ...
        QSqlQuery sql_query;
        QString selectsql="";
        selectsql="select * from student where sID=? and passwd=?";
        sql_query.prepare(selectsql);
        sql_query.bindValue(0,stuID);
        ...
        else
        {
            if(sql_query.next())
            {
                ...
                QString updatesql="update student set passwd=? where sID=?";
                sql_update.prepare(updatesql);
                sql_update.bindValue(0,indexnewpass);
                sql_update.bindValue(1,stuID);
                if(!sql_update.exec())
                {
...
void StuPassWidget::click_reset()
{
